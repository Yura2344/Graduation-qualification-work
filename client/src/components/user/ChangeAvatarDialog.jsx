...
export default function ChangeAvatarDialog({open, setIsOpen, username}){
  ...
  async function changeAvatar() {
    ...
    data.append("cropParams", JSON.stringify({
      cropX: scale * crop.x, 
      cropY: scale * crop.y, 
      cropWidth: scale * crop.width, 
      cropHeight: scale * crop.height
    }));
    axiosInstance.put(`/users/${username}/avatar`, data).then(async (res) => {
      await updateUser();
      setNewAvatarFile(null);
      enqueueSnackbar("Successfully changed avatar");
      setIsOpen(false);
    }).catch((err) => {
      console.log(err);
    });
